...
resource "azurerm_management_group" "landing_zone_management_groups" {
  for_each                   = { for k in var.landing_zones : "${azurerm_management_group.primary_management_groups["landing_zones"].name}-${k.name}" => k }
...
resource "azurerm_management_group_subscription_association" "landing_zone_associations" {
  for_each            = { for k in local.landing_zone_subscriptions : k.name => k if k != null }
  management_group_id = azurerm_management_group.landing_zone_management_groups["${azurerm_management_group.primary_management_groups["landing_zones"].name}-${each.value["parent"]}"].id
